...
require 'json'
def rps_tournament_winner(branch)

    unless branch[0][0].is_a? String
        branch[0] = rps_tournament_winner(branch[0])
        branch[1] = rps_tournament_winner(branch[1])
    end
    branch = rps_game_winner(branch)
...
def array_detect(array_string)
    ...
    if json.is_a? Array
        puts(rps_tournament_winner(json))
    else
        # not an array
    end
